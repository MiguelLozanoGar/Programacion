...
#creamos una clase que herede de Cliente por si es necesario utilizar algun dato de esta clase
class Producto(Cliente):
    ...
    def selecionProducto(self):
        print(f'Ha añadido a la cesta el producto {self.nombreProducto} cuyo identificador es "{self.id}" se trata de {self.descripcion}, tiene un coste de {self.precio} euros por unidad ')
        cantidad=int(input('¿cuantas unidades desea? \n'))
        if cantidad <= self.cantidad:
            print('unidades añadidas')
        else:
            print('no disponemos de suficientes existencias') 
...
while continuar:
    ...
    if selecProducto =='a' or selecProducto == 'A':
            ...
            int(input('introduzca su numero de tarjeta \n'))
            print('se le cargara el importe de su compra a la tarjeta recien introducida')
